    ...
    public readonly struct PackagesInstallerStep: IPatcherStep {
        public UniTask<StepResult> Run() {
            ...
            var allPackages = packages
                .Concat(gitPackages.Select(x => new FoundDllInfo(x.version, null, null, PackageMatchType.Exact)))
                .ToArray();
            // var gitPackages = settings.GitPackages;
        ...
        private bool ManuallyResolveManifest() {
            ...
            var packages = settings.ExactPackagesFound;
            var gitPackages = settings.GitPackages;
            var allPackages = packages.Concat(gitPackages.Select(x => new FoundDllInfo(x.name, x.version, null, PackageMatchType.Exact)));
